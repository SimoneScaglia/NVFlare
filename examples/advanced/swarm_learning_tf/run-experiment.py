...
def run_command(command, shell=True):
    ...
    print(f"\nExecuting: {command}")
    # with git bash
    result = subprocess.run(command, shell=shell, capture_output=True, text=True, cwd=script_dir, executable='/bin/bash')
...
def copy_data_files(data_dir, iteration, num_nodes):
    ...
    for i in range(1, num_nodes + 1):
        src_file = os.path.join(data_dir, f"node{i}_{iteration}.csv")
...
def main():
    ...
    copy_data_files(data_directory, iteration, num_nodes)
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    val_script = os.path.join(script_dir, "../mimic/utils/val_glob_model_slost.py")
    
    if os.path.exists(val_script):
        run_command(f"python3 \"{val_script}\" \"{json_path}\"")
    else:
        # Try alternative path
        val_script = os.path.join(os.path.dirname(script_dir), "mimic/utils/val_glob_model_slost.py")
        if os.path.exists(val_script):
            run_command(f"python3 \"{val_script}\" \"{json_path}\"")
        else:
