...
class MimicModelLearner(ModelLearner):
    ...
    def train(self, model: FLModel) -> Union[str, FLModel]:
        ...
        self.info(f"Current/Total Round: {round_num}/{self.total_rounds}")
        ...
        self.info(f"Validation metrics round {round_num}: {metrics_results}")

        # ===== Save metrics to fixed folder =====
        metrics_dir = "/tmp/nvflare/results/metrics"
        os.makedirs(metrics_dir, exist_ok=True)
        metrics_file = os.path.join(metrics_dir, f"metrics_node{self.site_name}_round{round_num}.json")
        with open(metrics_file, "w") as f:
            json.dump(metrics_results, f, indent=4)

        # ===== Save model weights to fixed folder =====
        weights_dir = "/tmp/nvflare/results/weights"
        os.makedirs(weights_dir, exist_ok=True)
        weights_file = os.path.join(weights_dir, f"node{self.site_name}_round{round_num}.weights.h5")
        self.model.save_weights(weights_file)

        # Update best model if needed
        if metrics_results["auc"] > self.best_auc:
