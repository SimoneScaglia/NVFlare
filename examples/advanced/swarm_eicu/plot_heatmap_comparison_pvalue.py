...
def load_mean_results(directories, csv_filename, learning_rates=None, batch_sizes=None):
    """Load mean results from directories with given CSV filename.
       For each file:
       - Group by iteration
       - Compute weighted AUC per iteration using 'splits' if they sum to 100
       - Otherwise use mean AUC for that iteration
       - Final result is the mean over iterations
    """
...
def create_triple_heatmap(comparison, metric, n_training_samples, n_test_samples):
    ...
    # Calculate the average of A and B for each cell
    avg_values = (conf1_values + conf2_values) / 2

    # Find the indices of the maximum average value
    max_avg_indices = np.where(avg_values == np.max(avg_values))

    # Log the combination with the highest average to the terminal
...
def try_plot_triple_comparison(comparison):
    ...
    try:
        print(f"Plotting triple comparison for: {comparison['exp']}")
...
if __name__ == "__main__":
    ...
    comparisons = [
        {
            'exp': 'NVFLARE_SWARM_CENTRAL_A = Swarm 5 nodes (3000 rows/node) vs B = Central (18000 rows) 70k var rows',
            'dir1': 'results/heatmap_experiments_5nodes',
            'file1': 'swarm_results.csv',
            'node1': 5,
            'dir2': 'results/heatmap_experiments_5nodes',
        ...
        {
            'exp': 'NVFLARE_SWARM_CENTRAL_A = Swarm 5 nodes (1200 rows/node) vs B = Central (6000 rows) 20k var rows',
            'dir1': 'results/heatmap_experiments_5nodes_20k',
            'file1': 'swarm_results.csv',
            'node1': 5,
            'dir2': 'results/heatmap_experiments_5nodes_20k',
            'file2': 'central_results.csv',
            'node2': 5,
        ...
        {
            'exp': 'NVFLARE_SWARM_CENTRAL_A = Swarm 5 nodes (2500 rows/node) vs B = Central (12500 rows) 70k fixed rows',
        ...
        {
            'exp': 'NVFLARE_SWARM_CENTRAL_A = Swarm 5 nodes (840 rows/node) vs B = Central (4200 rows) 20k fixed rows',
            'dir1': 'results/heatmap_experiments_5nodes_20k_fixed_rows',
            'file1': 'swarm_results.csv',
            'node1': 5,
            'dir2': 'results/heatmap_experiments_5nodes_20k_fixed_rows',
            'file2': 'central_results.csv',
            'node2': 5,
            'n_training_samples': 840,
            'n_test_samples': 360
        },
    ]
