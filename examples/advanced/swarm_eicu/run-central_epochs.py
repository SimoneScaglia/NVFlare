...
def train_and_eval(x_nodes: int, iteration_y: int, epochs: int, batch_size: int, verbose=0, out_file: Path | None = None):
    ...
    # Callback to evaluate on test set and append CSV each epoch
    class EvalAndLogCallback(keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            metrics = self.model.evaluate(X_test, y_test, batch_size=batch_size, verbose=0, return_dict=True)
            row = {
    ...
    model.fit(
        ...
        verbose=verbose,
        shuffle=True,
        callbacks=[EvalAndLogCallback()],
    )
    # Final evaluate (optional return)
...
def main():
    ...
    # Derive total epochs like the swarm setup if available
    EPOCHS = int(config.get("num_aggregation_rounds", 25) * config.get("aggregation_per_epoch", 1))
    ...
    # Train and log per-epoch metrics directly to CSV
    _ = train_and_eval(
    ...
    print(f"Salvati risultati per-epoca in {out_file}")
