#!/usr/bin/env python3
"""
Central training: merges data from all selected nodes, trains one model,
evaluates on the shared test set and saves metrics to CSV.

Usage:
    python run-central.py <num_nodes> <configuration> <iteration> <lr> <bs>
...
def main():
    parser = argparse.ArgumentParser(description="Central training for same-size experiments")
    parser.add_argument('num_nodes', type=int, help="Number of nodes to centralize (2..max)")
    ...
    # Merge data from all selected nodes
    all_dfs = []
