...
Usage:
    ...
    python eicu_run-local-adaptive.py --config 10nodes
"""
...
# ── Grid search ───────────────────────────────────────────────────────────────
def grid_search(config_name, available_hospitals):
    """
    ...
    # For each iteration, pick the first hospital from the selection
    gs_hospitals = []
    ...
    for bs in BATCH_SIZES:
        for lr in LRS:
            ...
            for idx, iteration in enumerate(iterations):
                ...
                print(f"\n  [{done}/{total}] BS={bs} LR={lr} Iter={iteration} Hospital={hid}")

                metrics = run_local_single_hospital(config_name, hid, lr, bs)
...
def run_local_all(config_name, max_nodes, lr, bs, available_hospitals):
    """
        ...
        for iteration in range(NUM_ITERATIONS):
            ...
            for hid in selected:
                ...
                if metrics is not None:
                    ...
                    print(f"  [Hospital-{hid}] AUC={metrics['auc']:.4f} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Adaptive local training for eICU with per-configuration grid search"
    )
    parser.add_argument(
        '--config',
    ...
    print(f"\n{'#'*60}")
    print(f"ADAPTIVE LOCAL TRAINING (eICU) - Started {datetime.now()}")
    ...
    for config_name, max_nodes in configs.items():
        ...
        best_lr, best_bs = grid_search(config_name, available_hospitals)

        # Phase 2: Run local for all num_nodes
        run_local_all(config_name, max_nodes, best_lr, best_bs, available_hospitals)
